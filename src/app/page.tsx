...
export default function Home() {
  ...
  const tabataElapsedSeconds = useMemo(() => {
    ...
    if (!isTimerRunning && !isTimerPreparing && currentTabataRound === 1 && tabataPhase === 'work') return 0;
    let elapsed = 0;
    // Finished rounds
  ...
  }, [timerMode, currentTabataRound, tabataPhase, timerSeconds, isTimerRunning, isTimerPreparing]);
  ...
  const speak = useCallback((text: string) => {
    ...
    // Throttle Ritual: Prevent repeating the same text within 1 second
    const now = Date.now();
    if (lastSpokenRef.current.text === text && now - lastSpokenRef.current.time < 1000) return;
    ...
    if (typeof window !== "undefined" && window.speechSynthesis) {
      ...
      utterance.rate = 0.8; // Authoritative, steady pace
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      bellAudioRef.current = new window.Audio("/ignition.wav"); 
      ignitionAudioRef.current = new window.Audio("/ignition.wav");
    }
  }, []);

  // TABATA ROUND & PHASE TRANSITION RITUAL
  useEffect(() => {
    if (timerMode !== 'tabata' || !isTimerRunning || isPaused) return;
    ...
    if (timerSeconds === 0) {
      if (tabataPhase === 'work') {
        if (currentTabataRound < tabataRounds) {
          ...
          if (!isVoiceMuted) speak("REST");
        } else {
          // FINAL ROUND COMPLETE -> Success Sequence
          ...
          if (!isVoiceMuted) speak("WORKOUT COMPLETE");
        }
      } else {
        ...
        setCurrentTabataRound(prev => prev + 1);
        if (bellAudioRef.current) { bellAudioRef.current.currentTime = 0; bellAudioRef.current.play().catch(() => {}); }
        if (!isVoiceMuted) speak("WORK");
      }
    }
  }, [timerSeconds, timerMode, isTimerRunning, isPaused, tabataPhase, currentTabataRound, tabataRounds, speak, isVoiceMuted, caloriesBurnedLive, tabataTotalDuration]);

  // COUNTDOWN COMPLETION RITUAL
  useEffect(() => {
    if (timerMode !== 'countdown' || !isTimerRunning || isPaused) return;
    if (timerSeconds === 0) {
      setIsTimerRunning(false);
      if (ignitionAudioRef.current) { ignitionAudioRef.current.currentTime = 0; ignitionAudioRef.current.play().catch(() => {}); }
      if (!isVoiceMuted) speak("COMPLETE");
    }
  }, [timerSeconds, timerMode, isTimerRunning, isPaused, speak, isVoiceMuted]);

  // WARNING BEEPS RITUAL (Last 3 seconds)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isTimerPreparing && !isPaused && timerPrepareTime === 0) {
  ...
  }, [timerPrepareTime, isTimerPreparing, isPaused, timerMode, speak, isVoiceMuted]);

  // INITIAL "GET READY" SPEECH
  useEffect(() => {
    if (isTimerPreparing && !isPaused && timerPrepareTime === 5) {
      if (!isVoiceMuted) speak("GET READY");
    }
  }, [isTimerPreparing, isPaused, timerPrepareTime, isVoiceMuted, speak]);

  // CORE TICK LOOP: Simple decrement/increment logic
  useEffect(() => {
    let interval: any;
    if ((isTimerRunning || isTimerPreparing) && !isPaused) {
      interval = setInterval(() => {
        if (isTimerPreparing) {
          setTimerPrepareTime(prev => Math.max(0, prev - 1));
        } else {
  ...
  }, [isTimerRunning, isTimerPreparing, isPaused, timerMode]);
;
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && (isTraining || isTimerRunning) && !isPaused) {
        requestWakeLock();
      }
    };
  ...
  }, [isTraining, isTimerRunning, isPaused, requestWakeLock]);
    ...
    <>
      ...
      {!(isTraining || isPreparing) ? (
        ...
        >
          ...
          {/* List Ritual: Filter Chips at top of Session */}
          {selectionMode === 'surprise' && session.length > 0 && (
            <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", padding: "1rem 1.5rem 1.5rem", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              {noEquip && (
                <span style={{ fontSize: "0.8rem", background: "rgba(255,255,255,0.1)", padding: "0.4rem 1.2rem", borderRadius: "100px", fontWeight: "900", letterSpacing: "0.05em" }}>BODYWEIGHT ONLY</span>
              )}
              {isSurfMode && (
          ...
          <div style={{ 
            ...
            <div style={{
              ...
              <div style={{ padding: "2rem 1.5rem 4rem 1.5rem", borderTop: "1px solid rgba(0,0,0,0.1)", background: "#daff00" }}>
                <button 
                  onClick={() => { setIsTrayExpanded(false); requestWakeLock(); startSession(); }}
                  style={{
                    background: "black",
                    color: "#daff00",
                    border: "none",
          ...
          <div style={{ 
            ...
            <div style={{ display: "flex", gap: "1rem" }}>
              {!hasCompletedCooldown && (
                <button 
                  onClick={() => { requestWakeLock(); startCooldown(); }}
                  style={{
                    flex: 1,
                    height: "88px",
                    borderRadius: "100px",
            ...
            {/* SAVE WORKOUT BUTTON AS TILE ACTION - Hidden for Tabata sessions */}
            {!finalTabataStats && (
              <>
        ...
        >
          ...
          <div style={{ 
            ...
            {isTimerPreparing ? timerPrepareTime : (
              (timerMode === 'tabata' && !isTimerRunning && !isTimerPreparing) 
                ? formatTime(tabataTotalDuration) 
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "2rem" }}>
                  <button 
                    ...
                    style={{ width: "56px", height: "56px", borderRadius: "50%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "white", fontSize: "1.5rem", fontWeight: "900", cursor: "pointer" }}
                  >
                    -
                  </button>
                  <div style={{ textAlign: "center" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "2rem", marginTop: "20px", paddingBottom: "180px" }}>
            <div style={{ width: "90px", display: "flex", justifyContent: "center" }}>
              <button 
                ...
                style={{ 
                  ...
                  height: "56px", 
                  borderRadius: "50%", 
                  background: isVoiceMuted ? "rgba(255,255,255,0.05)" : "rgba(0,0,0,0.5)", 
                  border: "1px solid rgba(255,255,255,0.1)", 
                  color: isVoiceMuted ? "rgba(255,255,255,0.3)" : "white", 
                  display: "flex", 
                  alignItems: "center", 
              ...
              >
                {isVoiceMuted ? (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="1" y1="1" x2="23" y2="23"></line><path d="M9 9l-5 5H2v-4h2l5 5zM11 5L6 10M11 19l-5-5"></path></svg>
                ) : (
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                    <path d="M24,0 L24,24 L0,24 L0,0 L24,0 Z M12.5934901,23.257841 L12.5819402,23.2595131 L12.5108777,23.2950439 L12.4918791,23.2987469 L12.4918791,23.2987469 L12.4767152,23.2950439 L12.4056548,23.2595131 C12.3958229,23.2563662 12.3870493,23.2590235 12.3821421,23.2649074 L12.3780323,23.275831 L12.360941,23.7031097 L12.3658947,23.7234994 L12.3769048,23.7357139 L12.4804777,23.8096931 L12.4953491,23.8136134 L12.4953491,23.8136134 L12.5071152,23.8096931 L12.6106902,23.7357139 L12.6232938,23.7196733 L12.6232938,23.7196733 L12.6266527,23.7031097 L12.609561,23.275831 C12.6075724,23.2657013 12.6010112,23.2592993 12.5934901,23.257841 L12.5934901,23.257841 Z M12.8583906,23.1452862 L12.8445485,23.1473072 L12.6598443,23.2396597 L12.6498822,23.2499052 L12.6498822,23.2499052 L12.6471943,23.2611114 L12.6650943,23.6906389 L12.6699349,23.7034178 L12.6699349,23.7034178 L12.678386,23.7104931 L12.8793402,23.8032389 C12.8914285,23.8068999 12.9022333,23.8029875 12.9078286,23.7952264 L12.9118235,23.7811639 L12.8776777,23.1665331 C12.8752882,23.1545897 12.8674102,23.1470016 12.8583906,23.1452862 L12.8583906,23.1452862 Z M12.1430473,23.1473072 C12.1332178,23.1423925 12.1221763,23.1452606 12.1156365,23.1525954 L12.1099173,23.1665331 L12.0757714,23.7811639 C12.0751323,23.7926639 12.0828099,23.8018602 12.0926481,23.8045676 L12.108256,23.8032389 L12.3092106,23.7104931 L12.3186497,23.7024347 L12.3186497,23.7024347 L12.3225043,23.6906389 L12.340401,23.2611114 L12.337245,23.2485176 L12.337245,23.2485176 L12.3277531,23.2396597 L12.1430473,23.1473072 Z" fillRule="nonzero" opacity="0" />
                    <path d="M12,2.5 C12.7796706,2.5 13.4204457,3.09488554 13.4931332,3.85553954 L13.5,4 L13.5,20 C13.5,20.8284 12.8284,21.5 12,21.5 C11.2203294,21.5 10.5795543,20.9050879 10.5068668,20.1444558 L10.5,20 L10.5,4 C10.5,3.17157 11.1716,2.5 12,2.5 Z M8,5.5 C8.82843,5.5 9.5,6.17157 9.5,7 L9.5,17 C9.5,17.8284 8.82843,18.5 8,18.5 C7.17157,18.5 6.5,17.8284 6.5,17 L6.5,7 C6.5,6.17157 7.17157,5.5 8,5.5 Z M16,5.5 C16.8284,5.5 17.5,6.17157 17.5,7 L17.5,17 C17.5,17.8284 16.8284,18.5 16,18.5 C15.1716,18.5 14.5,17.8284 14.5,17 L14.5,7 C14.5,6.17157 15.1716,5.5 16,5.5 Z M4,8.5 C4.82843,8.5 5.5,9.17157 5.5,10 L5.5,14 C5.5,14.8284 4.82843,15.5 4,15.5 C3.17157,15.5 2.5,14.8284 2.5,14 L2.5,10 C2.5,9.17157 3.17157,8.5 4,8.5 Z M20,8.5 C20.7796706,8.5 21.4204457,9.09488554 21.4931332,9.85553954 L21.5,10 L21.5,14 C21.5,14.8284 20.8284,15.5 20,15.5 C19.2203294,15.5 18.5795543,14.9050879 18.5068668,14.1444558 L18.5,14 L18.5,10 C18.5,9.17157 19.1716,8.5 20,8.5 Z" fill="currentColor" />
            ...
            <button 
              onClick={() => { 
                primeAudio();
                requestWakeLock(); // Request on gesture
              ...
              style={{
                width: (isTimerRunning || isTimerPreparing) ? "64px" : "90px",
                height: (isTimerRunning || isTimerPreparing) ? "64px" : "90px",
                borderRadius: "50%",
                background: (isTimerRunning || isTimerPreparing) ? "black" : "var(--accent)",
                color: (isTimerRunning || isTimerPreparing) ? "white" : "black",
                border: (isTimerRunning || isTimerPreparing) ? "1px solid rgba(255, 255, 255, 0.2)" : "medium",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                cursor: "pointer",
                boxShadow: (isTimerRunning || isTimerPreparing) ? "none" : "rgba(0, 0, 0, 0.15) 0px 10px 30px",
                transition: "0.3s",
                zIndex: 10
              }}
            >
              {isTimerRunning || isTimerPreparing ? (
                ...
                <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor" style={{ marginLeft: "4px" }}><polygon points="5 3 19 12 5 21 5 3"/></svg>
              )}
            </button>
